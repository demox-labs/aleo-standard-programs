...
describe('ARC0038', () => {
  ...
  const initializeUsers = (start: number, end: number): { block: number, user: string }[] => {
  ...
  it('withdraw batching', () => {
    ...
    const depositsAndWithdrawals = users.map((user) => {
      ...
      return [
        ...
        withdrawPublic(withdrawBlock, user.user, 1, 99.999999 * MICROCREDITS_TO_CREDITS, shouldFail)
      ];
    ...
    const transitions = [
      ...
      claimUnbond(1000),
      claimWithdrawPublic(1000, user0, 99.999999 * MICROCREDITS_TO_CREDITS, user0, false),
      claimWithdrawPublic(1000, users[0].user, 99.999999 * MICROCREDITS_TO_CREDITS, users[0].user, false),
