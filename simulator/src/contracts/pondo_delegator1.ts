...
export class delegator1Program {
  ...
  address: string = 'delegator1.aleo';
  block: {
    height: bigint;
  } = { height: BigInt(0) };
  // params
  ...
  //program delegator1.aleo {// There are 4 potential states for a delegator:
  ...
  bond(validator: string, amount: bigint) {
    ...
    this.credits.caller = 'delegator1.aleo';
    this.credits.bond_public(validator, 'delegator1.aleo', amount);
  ...
  finalize_bond(validator: string) {
    ...
    let balance: bigint = BigInt.asUintN(
      64,
      this.credits.account.get('delegator1.aleo')!
    );
  ...
  unbond(amount: bigint) {
    ...
    this.credits.signer = this.signer;
    this.credits.caller = 'delegator1.aleo';
    this.credits.unbond_public('delegator1.aleo', amount);
  ...
  finalize_unbond() {
    ...
    let bonded: boolean = this.credits.bonded.has('delegator1.aleo');
  ...
  ban_validator(validator: string) {
    ...
    this.validator_oracle.caller = 'delegator1.aleo';
