...
// Ensure the delegator is in the correct state
  ...
  finalize_bond(
    ...
    let balance: bigint = this.credits.account.get("pondo_delegator3.aleo")!;
...
// Should be entirely unbonded
    let bonded: boolean = this.credits.bonded.has("pondo_delegator3.aleo");
    assert(bonded === false);
...
// Assume someone called claim_unbond_public
  ...
  finalize_terminal_state(
    ...
    let bonded: boolean = this.credits.bonded.has("pondo_delegator3.aleo");
    assert(bonded === false);
    let is_unbonding: boolean = this.credits.unbonding.has("pondo_delegator3.aleo");
