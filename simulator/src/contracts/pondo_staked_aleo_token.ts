...
export class pondo_staked_aleo_tokenProgram {
  ...
  register_token() {
    ...
    let name: bigint = BigInt('1631421259099656974472467909989204'); // "Pondo Aleo LST" ascii encoded
    let symbol: bigint = BigInt('482131854671'); // "pALEO" ascii encoded
  ...
  mint_public(amount: bigint, receiver: string) {
    assert(this.caller === 'pondo_core_protocol.aleo');
  ...
  burn_public(amount: bigint, owner: string) {
    assert(this.caller === 'pondo_core_protocol.aleo');
