...
export class pondo_oracleProgram {
  ...
  INITIAL_DELEGATOR_APPROVER_ADDRESS = "aleo1am58znyhghvyj7lesu0h6wvxecxfhu8svdvgema6g5eqv7kecuzsm7z039";
...
// Set the control addresses
    ...
    delegator_allocation.set(BigInt("0"), [
    BigInt("370"),
    BigInt("260"),
    BigInt("160"),
    BigInt("120"),
...
// Ensure a program is calling
  ...
  finalize_propose_delegator(
    reference_delegator: string,
    validator: string,
  ) {
// Ensure the validator isn't banned
    ...
    let contains_delegator: boolean = this.delegator_to_validator.has(reference_delegator);
...
// Check that proposed_reference_delegator contains the reference delegator
    let contains_delegator: boolean = this.delegator_to_validator.has(delegator);
...
// Get the validator address and ensure the delegator is bonded to the validator
    let proposed_validator_address: string = this.delegator_to_validator.get(delegator)!;
    let bonded: bond_state = this.credits.bonded.get(delegator)!;
    assert(bonded.validator === proposed_validator_address);
    
// Ensure the validator isn't banned
    let is_banned: boolean = this.banned_validators.has(proposed_validator_address);
    assert(is_banned === false);
    
// Ensure the reference delegator is not already part of the reference delegators
    let contains_validator_reference: boolean = this.validator_data.has(proposed_validator_address);
    assert(contains_validator_reference === false);
    
// Get the commission from the committee state
    let validator_committee_state: committee_state = this.credits.committee.get(proposed_validator_address)!;
...
// It's permissionless ie callable by anyone
  ...
  finalize_update_data(
    delegator: string,
  ) {
// Get the existing data, fails if the reference delegator isn't there
    let existing_validator_datum: validator_datum = this.validator_data.get(delegator)!;
    
// Ensure the validator isn't banned
    let is_banned: boolean = this.banned_validators.has(existing_validator_datum.validator);
    assert(is_banned === false);
    
// Check if update is in the allowed update period
    let epoch_blocks: bigint = this.block.height % this.BLOCKS_PER_EPOCH;
    let is_update_period: boolean = epoch_blocks >= this.UPDATE_BLOCKS_DISALLOWED;
    assert(is_update_period);
    
// Ensure an update hasn't been performed in the same epoch yet
    let block_range: bigint = this.block.height - existing_validator_datum.block_height;
    assert(block_range > this.UPDATE_BLOCKS_DISALLOWED);
    
// Get the committee state of the validator
    let validator_committee_state: committee_state = this.credits.committee.get(existing_validator_datum.validator)!;
...
// So the microcredits_yield_per_epoch would be 18_345_688
    let microcredits_earned: bigint = bonded.microcredits - existing_validator_datum.bonded_microcredits;
    let normalized_microcredits_earned: bigint = microcredits_earned * this.PRECISION / existing_validator_datum.bonded_microcredits;
    let yield_per_epoch: bigint = normalized_microcredits_earned * this.BLOCKS_PER_EPOCH / block_range;
    
// Get the boost amount for the validator
    let boost: validator_boost = this.validator_boosting.get(undefined) || undefinedexisting_validator_datum.validator, validator_boost {
    epoch: BigInt("0"),
    boost_amount: BigInt("0")
    });
    let boost_amount: bigint = boost.epoch == current_epoch ? boost.boost_amount : BigInt("0");
// Normalize the boost amount by the pondo tvl
    let current_pondo_tvl: bigint = this.pondo_tvl.get(BigInt("0")) || BigInt("10000000000000000"); // use a high default, 10B credits
...
// Note: This precision is 1 credit on a TVL of 10M credits
    let normalized_boost_amount: bigint = boost_amount * this.PRECISION / current_pondo_tvl;
...
// The attack here is to prevent a validator from keeping many reference delegators and then choosing the most favorable range.
    let previous_update_epoch: bigint = existing_validator_datum.block_height / this.BLOCKS_PER_EPOCH;
    let did_update_last_epoch: boolean = (previous_update_epoch + BigInt("1")) == current_epoch;
    let new_microcredits_yield_per_epoch: bigint = did_update_last_epoch ? yield_per_epoch + normalized_boost_amount : BigInt("0");
...
// Get the array of top validators
    let top_validators_addresses: string[] = this.top_validators.get(undefined) || undefined
    BigInt("0"),
    [
...
// Fetch all of the data for each validator
    let datum_0: validator_datum = this.validator_data.get(undefined) || undefinedtop_validators_addresses[0], default_validator_datum);
    let datum_1: validator_datum = this.validator_data.get(undefined) || undefinedtop_validators_addresses[1], default_validator_datum);
    let datum_2: validator_datum = this.validator_data.get(undefined) || undefinedtop_validators_addresses[2], default_validator_datum);
    let datum_3: validator_datum = this.validator_data.get(undefined) || undefinedtop_validators_addresses[3], default_validator_datum);
    let datum_4: validator_datum = this.validator_data.get(undefined) || undefinedtop_validators_addresses[4], default_validator_datum);
    let datum_5: validator_datum = this.validator_data.get(undefined) || undefinedtop_validators_addresses[5], default_validator_datum);
    let datum_6: validator_datum = this.validator_data.get(undefined) || undefinedtop_validators_addresses[6], default_validator_datum);
    let datum_7: validator_datum = this.validator_data.get(undefined) || undefinedtop_validators_addresses[7], default_validator_datum);
    let datum_8: validator_datum = this.validator_data.get(undefined) || undefinedtop_validators_addresses[8], default_validator_datum);
    let datum_9: validator_datum = this.validator_data.get(undefined) || undefinedtop_validators_addresses[9], default_validator_datum);
...
// The order of the this.inline_swap_validator_data is subtle but very important.
    let swap_result_0: [validator_datum, validator_datum, boolean] = this.inline_swap_validator_data(new_validator_datum, datum_0, epoch_start_height, false, allocations[0]);
    let swap_result_1: [validator_datum, validator_datum, boolean] = this.inline_swap_validator_data(swap_result_0[1], datum_1, epoch_start_height, swap_result_0[2], allocations[1]);
    let swap_result_2: [validator_datum, validator_datum, boolean] = this.inline_swap_validator_data(swap_result_1[1], datum_2, epoch_start_height, swap_result_1[2], allocations[2]);
    let swap_result_3: [validator_datum, validator_datum, boolean] = this.inline_swap_validator_data(swap_result_2[1], datum_3, epoch_start_height, swap_result_2[2], allocations[3]);
    let swap_result_4: [validator_datum, validator_datum, boolean] = this.inline_swap_validator_data(swap_result_3[1], datum_4, epoch_start_height, swap_result_3[2], allocations[4]);
    let swap_result_5: [validator_datum, validator_datum, boolean] = this.inline_swap_validator_data(swap_result_4[1], datum_5, epoch_start_height, swap_result_4[2], allocations[5]);
    let swap_result_6: [validator_datum, validator_datum, boolean] = this.inline_swap_validator_data(swap_result_5[1], datum_6, epoch_start_height, swap_result_5[2], allocations[6]);
    let swap_result_7: [validator_datum, validator_datum, boolean] = this.inline_swap_validator_data(swap_result_6[1], datum_7, epoch_start_height, swap_result_6[2], allocations[7]);
    let swap_result_8: [validator_datum, validator_datum, boolean] = this.inline_swap_validator_data(swap_result_7[1], datum_8, epoch_start_height, swap_result_7[2], allocations[8]);
    let swap_result_9: [validator_datum, validator_datum, boolean] = this.inline_swap_validator_data(swap_result_8[1], datum_9, epoch_start_height, swap_result_8[2], allocations[9]);
...
// This protects against a DOS against other validators who could keep a delegator to another validator and then remove it right at the end of the update period
    let epoch_blocks: bigint = this.block.height % this.BLOCKS_PER_EPOCH;
    let is_not_update_period: boolean = epoch_blocks < this.UPDATE_BLOCKS_DISALLOWED;
    assert(is_not_update_period);
    
// Remove from the proposed_delegators if there
    let contains_delegator: boolean = this.delegator_to_validator.has(delegator_address);
    if (contains_delegator) {
    this.delegator_to_validator.delete(delegator_address);
    }
    
// Remove from the validator_data
    let data_contains_delegator: boolean = this.validator_data.has(delegator_address);
    if (data_contains_delegator) {
    this.validator_data.delete(delegator_address);
    }
    
// Remove from the top 10 validators if there
    let top_validators_addresses: string[] = this.top_validators.get(BigInt("0"))!;
    let new_validator_0: string = top_validators_addresses[0] == delegator_address ? "aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc" : top_validators_addresses[0];
    let new_validator_1: string = top_validators_addresses[1] == delegator_address ? "aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc" : top_validators_addresses[1];
    let new_validator_2: string = top_validators_addresses[2] == delegator_address ? "aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc" : top_validators_addresses[2];
    let new_validator_3: string = top_validators_addresses[3] == delegator_address ? "aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc" : top_validators_addresses[3];
    let new_validator_4: string = top_validators_addresses[4] == delegator_address ? "aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc" : top_validators_addresses[4];
    let new_validator_5: string = top_validators_addresses[5] == delegator_address ? "aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc" : top_validators_addresses[5];
    let new_validator_6: string = top_validators_addresses[6] == delegator_address ? "aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc" : top_validators_addresses[6];
    let new_validator_7: string = top_validators_addresses[7] == delegator_address ? "aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc" : top_validators_addresses[7];
    let new_validator_8: string = top_validators_addresses[8] == delegator_address ? "aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc" : top_validators_addresses[8];
    let new_validator_9: string = top_validators_addresses[9] == delegator_address ? "aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc" : top_validators_addresses[9];
    
// Swap until 0group address is at the end of the array
    let swap_result_0: string[] = this.inline_swap_zero_group_address(new_validator_0, new_validator_1);
    let swap_result_1: string[] = this.inline_swap_zero_group_address(swap_result_0[1], new_validator_2);
    let swap_result_2: string[] = this.inline_swap_zero_group_address(swap_result_1[1], new_validator_3);
    let swap_result_3: string[] = this.inline_swap_zero_group_address(swap_result_2[1], new_validator_4);
    let swap_result_4: string[] = this.inline_swap_zero_group_address(swap_result_3[1], new_validator_5);
    let swap_result_5: string[] = this.inline_swap_zero_group_address(swap_result_4[1], new_validator_6);
    let swap_result_6: string[] = this.inline_swap_zero_group_address(swap_result_5[1], new_validator_7);
    let swap_result_7: string[] = this.inline_swap_zero_group_address(swap_result_6[1], new_validator_8);
    let swap_result_8: string[] = this.inline_swap_zero_group_address(swap_result_7[1], new_validator_9);
...
// Get the validator address
    let validator: string = this.delegator_to_validator.get(reference_delegator)!;
...
// Get the committee state of the validator
    let validator_committee_state: committee_state = this.credits.committee.get(validator) || default_committee_state;
// Ensure the validator is open & the commission is less than MAX_COMMISSION
    assert(!validator_committee_state.is_open || validator_committee_state.commission > this.MAX_COMMISSION);
...
// Assert that the validator is in the committee, will fail
    let committee_state_contains_validator: boolean = this.credits.committee.has(validator);
...
// Transfer credits to the pondo core protocol
    ...
    this.credits.transfer_public_as_signer("pondo_core_protocol.aleo", boost_amount);
    return this.finalize_boost_validator( validator, boost_amount);
    }
    
  finalize_boost_validator(
    validator: string,
    boost_amount: bigint,
  ) {
...
// Get the current boosting for the validator
    let current_boost: validator_boost = this.validator_boosting.get(undefined) || undefinedvalidator, validator_boost {
    epoch: BigInt("0"),
    boost_amount: BigInt("0")
    });
    
// If the boost is in the same epoch, add the boost amount
    let new_boost_amount: bigint = current_boost.epoch == current_epoch ? current_boost.boost_amount + boost_amount : boost_amount;
...
// If the boost multiple is too low, it would be more profitable to decrease commission than to boost ie no one would ever boost unless commissions were 0s
    let first_yield: bigint = datum_0.microcredits_yield_per_epoch + datum_0.boost * this.BOOST_PRECISION / boost_multiple;
    let second_yield: bigint = datum_1.microcredits_yield_per_epoch + datum_1.boost * this.BOOST_PRECISION / boost_multiple;
...
// all the while paying transaction fees while giving their competitors commissions from delegations.
    let should_swap: boolean = new_auto_swap ? first_yield > second_yield : first_yield < second_yield;
...
// Swap the zero group address to the end of the list
  inline_swap_zero_group_address(
    address_0: string,
    address_1: string,
  ) {
