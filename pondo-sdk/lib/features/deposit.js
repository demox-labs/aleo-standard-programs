...
export async function depositPublic(
  ...
  const inputs = await getDepositPublicInputs(
    rpcProvider, address, depositCredits, referrer
  );
...
async function getDepositPublicInputs(
  rpcProvider, aleoAddress, depositCredits, referrer
) {
  ...
  contracts.coreProtocolInstance.deposit_public_as_signer(
    BigInt(depositMicrocredits),
    BigInt("1"),
    ''
  );
