...
export const programIdToAddress = Object.fromEntries(
  Object
    .entries(PROGRAMS)
    .map(
      ([programKey, program]) => ([program.id, program.address])
    )
);

export const programAddressToId = Object.fromEntries(
  Object
    .entries(PROGRAMS)
    .map(
      ([programKey, program]) => ([program.address, program.id,])
    )
);