...
export class pondo_delegator3Program {
    signer = 'not set';
    caller = 'not set';
    address = 'pondo_delegator3.aleo';
    block = { height: BigInt(0) };
    // params
    banned_validators = new Map();
    validator_mapping = new Map();
    state_mapping = new Map();
    ...
    constructor(
    // constructor args
    pondo_oracleContract, creditsContract) {
        // constructor body
    ...
    //program pondo_delegator3.aleo {// There are 4 potential states for a delegator:
    ...
    initialize() {
        assert(this.caller === 'pondo_core_protocol.aleo', 'Assert that the caller is the pondo core protocol');
    ...
    finalize_prep_rebalance() {
        let current_state = BigInt.asUintN(8, this.state_mapping.get(BigInt('0')));
    ...
    set_validator(new_validator, new_commission) {
        assert(this.caller === 'pondo_core_protocol.aleo', 'Assert that the caller is the pondo core protocol');
        assert(new_commission <= this.MAX_COMMISSION, 'Ensure the commission is within the allowed range');
        return this.finalize_set_validator(new_validator, new_commission);
    }
    finalize_set_validator(new_validator, new_commission) {
        // Set the new validator
        let next_validator_state = {
        ...
        let current_state = BigInt.asUintN(8, this.state_mapping.get(BigInt('0')));
        assert(current_state !== undefined);
        assert(current_state == this.TERMINAL, 'Ensure the delegator is in the this.TERMINAL state');
        this.state_mapping.set(BigInt('0'), this.BOND_ALLOWED);
    }
    bond(validator, amount) {
        this.credits.signer = this.signer;
        this.credits.caller = 'pondo_delegator3.aleo';
        this.credits.bond_public(validator, 'pondo_delegator3.aleo', amount);
        return this.finalize_bond(validator);
    }
    finalize_bond(validator) {
        let current_validator_state = this.validator_mapping.get(BigInt('0'));
        assert(current_validator_state !== undefined);
        assert(validator === current_validator_state.validator, "Ensure we're bonding to the correct validator");
        let balance = BigInt.asUintN(64, this.credits.account.get('pondo_delegator3.aleo'));
        assert(balance !== undefined);
        assert(balance === BigInt('0'), 'Ensure the delegator is bonded completely');
        let current_state = BigInt.asUintN(8, this.state_mapping.get(BigInt('0')));
        assert(current_state !== undefined);
        assert(current_state == this.BOND_ALLOWED ||
            current_state == this.UNBOND_NOT_ALLOWED, 'Ensure the delegator is in the correct state');
    ...
    unbond(amount) {
        ...
        this.credits.signer = this.signer;
        this.credits.caller = 'pondo_delegator3.aleo';
        this.credits.unbond_public('pondo_delegator3.aleo', amount);
        return this.finalize_unbond();
    }
    finalize_unbond() {
        // Should be entirely unbonded
        let bonded = this.credits.bonded.has('pondo_delegator3.aleo');
        ...
        let validator_committee_state = this.credits.committee.get(current_validator_state.validator) ||
            default_committee_state;
        // Check if the commission increased by more than the allowed amount
        let commission_increased = validator_committee_state.commission >
            current_validator_state.commission + this.MAX_COMMISSION_INCREASE;
        let commission_beyond_limit = validator_committee_state.commission > this.MAX_COMMISSION;
        ...
        if (commission_increased || commission_beyond_limit) {
            ...
            let current_state = BigInt.asUintN(8, this.state_mapping.get(BigInt('0')));
            assert(current_state !== undefined);
            assert(current_state === this.UNBOND_ALLOWED, 'Ensure the delegator is allowed to unbond');
        }
    ...
    finalize_terminal_state() {
        let bonded = this.credits.bonded.has('pondo_delegator3.aleo');
        assert(bonded === false, 'Ensure the delegator was completely unbonded');
        let is_unbonding = this.credits.unbonding.has('pondo_delegator3.aleo');
        assert(is_unbonding === false, 'Ensure the delegator is no longer unbonding');
        let current_state = BigInt.asUintN(8, this.state_mapping.get(BigInt('0')));
        assert(current_state !== undefined);
        assert(current_state != this.BOND_ALLOWED, 'Ensure the delegator is not in the this.BOND_ALLOWED state');
        if (current_state == this.UNBOND_NOT_ALLOWED) {
            let current_validator_state = this.validator_mapping.get(BigInt('0'));
            assert(current_validator_state !== undefined);
            this.banned_validators.set(current_validator_state.validator, true);
    ...
    transfer_to_core_protocol(amount) {
        assert(this.caller === 'pondo_core_protocol.aleo', 'Assert that the caller is the pondo core protocol');
        this.credits.signer = this.signer;
        this.credits.caller = 'pondo_delegator3.aleo';
    ...
    finalize_bond_failed() {
        ...
        let validator_is_unbonding = this.credits.unbonding.has(current_validator_state.validator);
        ...
        assert(validator_committee_state.is_open == false || validator_is_unbonding, 'Ensure the validator is closed to delegators or unbonding');
        let current_state = BigInt.asUintN(8, this.state_mapping.get(BigInt('0')));
        assert(current_state !== undefined);
        assert(current_state == this.BOND_ALLOWED, 'Ensure the delegator was in the this.BOND_ALLOWED state');
        this.state_mapping.set(BigInt('0'), this.TERMINAL);
    ...
    finalize_insufficient_balance() {
        ...
        let balance = BigInt.asUintN(64, this.credits.account.get('pondo_delegator3.aleo'));
        assert(balance !== undefined);
        assert(balance < BigInt('10000000000'), 'Ensure the balance is less than the minimum required to bond');
        let current_state = BigInt.asUintN(8, this.state_mapping.get(BigInt('0')));
        assert(current_state !== undefined);
        assert(current_state == this.BOND_ALLOWED, 'Ensure the delegator was in the this.BOND_ALLOWED state');
        this.state_mapping.set(BigInt('0'), this.TERMINAL);
    }
    ban_validator(validator) {
        this.pondo_oracle.signer = this.signer;
        this.pondo_oracle.caller = 'pondo_delegator3.aleo';
        this.pondo_oracle.pondo_ban_validator(validator);
        return this.finalize_ban_validator(validator);
    }
    finalize_ban_validator(validator) {
        assert(this.banned_validators.has(validator), 'Ensure the validator was in the banned mapping');
    }
}
