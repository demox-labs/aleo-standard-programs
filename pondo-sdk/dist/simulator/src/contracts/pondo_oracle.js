...
export class pondo_oracleProgram {
    signer = 'not set';
    caller = 'not set';
    address = 'pondo_oracle.aleo';
    block = { height: BigInt(0) };
    // params
    delegator_allocation = new Map();
    control_addresses = new Map();
    validator_boosting = new Map();
    pondo_tvl = new Map();
    banned_validators = new Map();
    top_validators = new Map();
    validator_data = new Map();
    delegator_to_validator = new Map();
    ...
    add_control_address(control_address) {
        return this.finalize_add_control_address(control_address, this.caller);
    }
    finalize_add_control_address(control_address, caller) {
        // Ensure the caller is an admin
        let is_admin = this.control_addresses.get(caller);
        assert(is_admin !== undefined);
        assert(is_admin);
        this.control_addresses.set(control_address, false);
    }
    remove_control_address(control_address) {
        return this.finalize_remove_control_address(control_address, this.caller);
    }
    finalize_remove_control_address(control_address, caller) {
        // Ensure the caller is an admin
        let is_admin = this.control_addresses.get(caller);
        assert(is_admin !== undefined);
        assert(is_admin);
    ...
    finalize_update_admin(new_admin, caller) {
        // Ensure the caller is an admin
        let is_admin = this.control_addresses.get(caller);
        assert(is_admin !== undefined);
        assert(is_admin);
    ...
    update_delegator_allocations(multiple) {
        return this.finalize_update_delegator_allocations(multiple, this.caller);
    }
    finalize_update_delegator_allocations(multiple, caller) {
        // Ensure the caller is an admin
        let is_admin = this.control_addresses.get(caller);
        assert(is_admin !== undefined);
        assert(is_admin);
    ...
    propose_delegator(validator) {
        // Ensure a program is calling
        assert(this.caller !== this.signer);
        return this.finalize_propose_delegator(this.caller, validator);
    }
    finalize_propose_delegator(reference_delegator, validator) {
        // Ensure the validator isn't banned
        let is_banned = this.banned_validators.has(validator);
        assert(is_banned === false);
        let contains_delegator = this.delegator_to_validator.has(reference_delegator);
    ...
    finalize_add_delegator(delegator, caller) {
        ...
        let proposed_validator_address = this.delegator_to_validator.get(delegator);
        assert(proposed_validator_address !== undefined);
        let bonded = this.credits.bonded.get(delegator);
        assert(bonded !== undefined);
        assert(bonded.validator === proposed_validator_address);
        // Ensure the validator isn't banned
        let is_banned = this.banned_validators.has(proposed_validator_address);
        assert(is_banned === false);
        // Ensure the reference delegator is not already part of the reference delegators
        let delegator_already_added = this.validator_data.has(delegator);
        assert(delegator_already_added === false);
        // Get the commission from the committee state
        let validator_committee_state = this.credits.committee.get(proposed_validator_address);
    ...
    update_data(delegator) {
        return this.finalize_update_data(delegator);
    }
    finalize_update_data(delegator) {
        // Get the existing data, fails if the reference delegator isn't there
        let existing_validator_datum = this.validator_data.get(delegator);
        assert(existing_validator_datum !== undefined);
        // Ensure the validator isn't banned
        let is_banned = this.banned_validators.has(existing_validator_datum.validator);
        assert(is_banned === false);
        // Check if update is in the allowed update period
        let epoch_blocks = BigInt.asUintN(32, this.block.height % this.BLOCKS_PER_EPOCH);
        let is_update_period = epoch_blocks >= this.UPDATE_BLOCKS_DISALLOWED;
        assert(is_update_period);
        // Ensure an update hasn't been performed in the same epoch yet
        let block_range = BigInt.asUintN(32, this.block.height - existing_validator_datum.block_height);
        assert(block_range > this.UPDATE_BLOCKS_DISALLOWED);
        // Get the committee state of the validator
        let validator_committee_state = this.credits.committee.get(existing_validator_datum.validator);
        assert(validator_committee_state !== undefined);
        ...
        let microcredits_earned = BigInt.asUintN(128, bonded.microcredits - existing_validator_datum.bonded_microcredits);
        let normalized_microcredits_earned = BigInt.asUintN(128, (microcredits_earned * this.PRECISION) /
            existing_validator_datum.bonded_microcredits);
        let yield_per_epoch = BigInt.asUintN(128, (normalized_microcredits_earned * this.BLOCKS_PER_EPOCH) / block_range);
        // Get the boost amount for the validator
        let boost = this.validator_boosting.get(existing_validator_datum.validator) || { epoch: BigInt('0'), boost_amount: BigInt('0') };
        let boost_amount = BigInt.asUintN(128, boost.epoch == current_epoch ? boost.boost_amount : BigInt('0'));
        // Normalize the boost amount by the pondo tvl
        let current_pondo_tvl = BigInt.asUintN(128, this.pondo_tvl.get(BigInt('0')) || BigInt('10000000000000000')); // use a high default, 10B credits
        ...
        let normalized_boost_amount = BigInt.asUintN(128, (boost_amount * this.PRECISION) / current_pondo_tvl);
        ...
        let previous_update_epoch = BigInt.asUintN(32, existing_validator_datum.block_height / this.BLOCKS_PER_EPOCH);
        let did_update_last_epoch = previous_update_epoch + BigInt('1') == current_epoch;
        let new_microcredits_yield_per_epoch = BigInt.asUintN(128, did_update_last_epoch
            ? yield_per_epoch + normalized_boost_amount
            : BigInt('0'));
        // Construct and save the new validator_datum for the delegator
        let new_validator_datum = {
            delegator: delegator,
        ...
        let top_validators_addresses = this.top_validators.get(BigInt('0'));
        let default_validator_datum = {
        ...
        let datum_0 = this.validator_data.get(top_validators_addresses[0]) ||
            default_validator_datum;
        let datum_1 = this.validator_data.get(top_validators_addresses[1]) ||
            default_validator_datum;
        let datum_2 = this.validator_data.get(top_validators_addresses[2]) ||
            default_validator_datum;
        let datum_3 = this.validator_data.get(top_validators_addresses[3]) ||
            default_validator_datum;
        let datum_4 = this.validator_data.get(top_validators_addresses[4]) ||
            default_validator_datum;
        let datum_5 = this.validator_data.get(top_validators_addresses[5]) ||
            default_validator_datum;
        let datum_6 = this.validator_data.get(top_validators_addresses[6]) ||
            default_validator_datum;
        let datum_7 = this.validator_data.get(top_validators_addresses[7]) ||
            default_validator_datum;
        let datum_8 = this.validator_data.get(top_validators_addresses[8]) ||
            default_validator_datum;
        let datum_9 = this.validator_data.get(top_validators_addresses[9]) ||
            default_validator_datum;
        // Calculate the epoch start block
        let epoch_start_height = BigInt.asUintN(32, current_epoch * this.BLOCKS_PER_EPOCH);
        // Get the boost multiple
        let allocations = this.delegator_allocation.get(BigInt('0'));
        // Perform swaps and drop the last element
        // The order of the this.inline_swap_validator_data is subtle but very important.
        let swap_result_0 = this.inline_swap_validator_data(new_validator_datum, datum_0, epoch_start_height, false, allocations[0]);
        let swap_result_1 = this.inline_swap_validator_data(swap_result_0[1], datum_1, epoch_start_height, swap_result_0[2], allocations[1]);
        let swap_result_2 = this.inline_swap_validator_data(swap_result_1[1], datum_2, epoch_start_height, swap_result_1[2], allocations[2]);
        let swap_result_3 = this.inline_swap_validator_data(swap_result_2[1], datum_3, epoch_start_height, swap_result_2[2], allocations[3]);
        let swap_result_4 = this.inline_swap_validator_data(swap_result_3[1], datum_4, epoch_start_height, swap_result_3[2], allocations[4]);
        let swap_result_5 = this.inline_swap_validator_data(swap_result_4[1], datum_5, epoch_start_height, swap_result_4[2], allocations[5]);
        let swap_result_6 = this.inline_swap_validator_data(swap_result_5[1], datum_6, epoch_start_height, swap_result_5[2], allocations[6]);
        let swap_result_7 = this.inline_swap_validator_data(swap_result_6[1], datum_7, epoch_start_height, swap_result_6[2], allocations[7]);
        let swap_result_8 = this.inline_swap_validator_data(swap_result_7[1], datum_8, epoch_start_height, swap_result_7[2], allocations[8]);
        let swap_result_9 = this.inline_swap_validator_data(swap_result_8[1], datum_9, epoch_start_height, swap_result_8[2], allocations[9]);
        let new_top_10 = [
    ...
    finalize_remove_delegator(delegator_address) {
        ...
        let epoch_blocks = BigInt.asUintN(32, this.block.height % this.BLOCKS_PER_EPOCH);
        let is_not_update_period = epoch_blocks < this.UPDATE_BLOCKS_DISALLOWED;
        assert(is_not_update_period);
        // Remove from the proposed_delegators if there
        let contains_delegator = this.delegator_to_validator.has(delegator_address);
        if (contains_delegator) {
            this.delegator_to_validator.delete(delegator_address);
        }
        // Remove from the validator_data
        let data_contains_delegator = this.validator_data.has(delegator_address);
        if (data_contains_delegator) {
            this.validator_data.delete(delegator_address);
        }
        // Remove from the top 10 validators if there
        let top_validators_addresses = this.top_validators.get(BigInt('0'));
        let new_validator_0 = top_validators_addresses[0] == delegator_address
            ? 'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc'
            : top_validators_addresses[0];
        let new_validator_1 = top_validators_addresses[1] == delegator_address
            ? 'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc'
            : top_validators_addresses[1];
        let new_validator_2 = top_validators_addresses[2] == delegator_address
            ? 'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc'
            : top_validators_addresses[2];
        let new_validator_3 = top_validators_addresses[3] == delegator_address
            ? 'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc'
            : top_validators_addresses[3];
        let new_validator_4 = top_validators_addresses[4] == delegator_address
            ? 'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc'
            : top_validators_addresses[4];
        let new_validator_5 = top_validators_addresses[5] == delegator_address
            ? 'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc'
            : top_validators_addresses[5];
        let new_validator_6 = top_validators_addresses[6] == delegator_address
            ? 'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc'
            : top_validators_addresses[6];
        let new_validator_7 = top_validators_addresses[7] == delegator_address
            ? 'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc'
            : top_validators_addresses[7];
        let new_validator_8 = top_validators_addresses[8] == delegator_address
            ? 'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc'
            : top_validators_addresses[8];
        let new_validator_9 = top_validators_addresses[9] == delegator_address
            ? 'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc'
            : top_validators_addresses[9];
        // Swap until 0group address is at the end of the array
        let swap_result_0 = this.inline_swap_zero_group_address(new_validator_0, new_validator_1);
        let swap_result_1 = this.inline_swap_zero_group_address(swap_result_0[1], new_validator_2);
        let swap_result_2 = this.inline_swap_zero_group_address(swap_result_1[1], new_validator_3);
        let swap_result_3 = this.inline_swap_zero_group_address(swap_result_2[1], new_validator_4);
        let swap_result_4 = this.inline_swap_zero_group_address(swap_result_3[1], new_validator_5);
        let swap_result_5 = this.inline_swap_zero_group_address(swap_result_4[1], new_validator_6);
        let swap_result_6 = this.inline_swap_zero_group_address(swap_result_5[1], new_validator_7);
        let swap_result_7 = this.inline_swap_zero_group_address(swap_result_6[1], new_validator_8);
        let swap_result_8 = this.inline_swap_zero_group_address(swap_result_7[1], new_validator_9);
        let new_top_validators_addresses = [
            swap_result_0[0],
            swap_result_1[0],
            swap_result_2[0],
            swap_result_3[0],
    ...
    pondo_ban_validator(validator) {
        return this.finalize_pondo_ban_validator(validator, this.caller);
    }
    finalize_pondo_ban_validator(validator, caller) {
        // Check if the caller is a control address
        let is_control_address = this.control_addresses.has(caller);
    ...
    ban_validator(reference_delegator) {
        return this.finalize_ban_validator(reference_delegator);
    }
    finalize_ban_validator(reference_delegator) {
        // Get the validator address
        let validator = this.delegator_to_validator.get(reference_delegator);
        ...
        let default_committee_state = {
            is_open: false,
            commission: this.MAX_COMMISSION + BigInt('1'),
        };
        // Get the committee state of the validator
        let validator_committee_state = this.credits.committee.get(validator) || default_committee_state;
    ...
    finalize_ban_self(validator, caller) {
        // Ensure the caller is the withdrawal address of the validator
        let withdraw_address = this.credits.withdraw.get(validator);
        assert(withdraw_address !== undefined);
        assert(withdraw_address === caller);
        // Assert that the validator is in the committee, will fail
        let committee_state_contains_validator = this.credits.committee.has(validator);
        assert(committee_state_contains_validator);
        // Ban the validator
        this.banned_validators.set(validator, true);
    }
    // Boost a validator
    boost_validator(validator, boost_amount) {
    ...
    finalize_boost_validator(validator, boost_amount) {
        ...
        let current_boost = this.validator_boosting.get(validator) || { epoch: BigInt('0'), boost_amount: BigInt('0') };
        // If the boost is in the same epoch, add the boost amount
        let new_boost_amount = BigInt.asUintN(64, current_boost.epoch == current_epoch
    ...
    inline_swap_validator_data(datum_0, datum_1, epoch_start_block, auto_swap, boost_multiple) {
        if (auto_swap) {
        ...
        let new_auto_swap = datum_0.validator == datum_1.validator;
        ...
        let first_yield = BigInt.asUintN(128, datum_0.microcredits_yield_per_epoch +
            (datum_0.boost * this.BOOST_PRECISION) / boost_multiple);
        let second_yield = BigInt.asUintN(128, datum_1.microcredits_yield_per_epoch +
    ...
    inline_swap_zero_group_address(address_0, address_1) {
        if (address_0 ==
            'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc') {
