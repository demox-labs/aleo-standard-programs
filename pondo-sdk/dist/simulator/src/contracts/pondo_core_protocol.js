...
export class pondo_core_protocolProgram {
    ...
    computed_paleo_for_deposit = BigInt("0");
    computed_credits_withdrawal = BigInt("0");
    pondo_delegator5;
    pondo_delegator4;
    pondo_delegator3;
    pondo_delegator2;
    ...
    constructor(
    // constructor args
    pondo_delegator5Contract, pondo_delegator4Contract, pondo_delegator3Contract, pondo_delegator2Contract, pondo_delegator1Contract, pondo_tokenContract, pondo_staked_aleo_tokenContract, pondo_oracleContract, multi_token_support_programContract, creditsContract) {
        // constructor body
    ...
    initialize(transfer_amount) {
        assert(transfer_amount >= BigInt('102'), 'Assert that the transfer amount is at least 102 microcredits, to ensure there is no division by zero, and that there is enough for the liquidity pool');
    ...
    finalize_initialize() {
        ...
        let top_validators = this.pondo_oracle.top_validators.get(BigInt('0')) || [
        ...
        let validator1 = this.pondo_oracle.validator_data.get(top_validators[0]) || default_datum;
        let validator2 = this.pondo_oracle.validator_data.get(top_validators[1]) || default_datum;
        let validator3 = this.pondo_oracle.validator_data.get(top_validators[2]) || default_datum;
        let validator4 = this.pondo_oracle.validator_data.get(top_validators[3]) || default_datum;
        let validator5 = this.pondo_oracle.validator_data.get(top_validators[4]) || default_datum;
    ...
    // DEPOSIT FUNCTIONS
    // -------------------
    deposit_public_as_signer(credits_deposit, expected_paleo_mint, referrer) {
        assert(expected_paleo_mint >= BigInt('1'), 'Assert that the expected pALEO mint is at least 1 microcredit');
        ...
        return this.finalize_deposit_public_as_signer(credits_deposit, expected_paleo_mint);
    }
    finalize_deposit_public_as_signer(credits_deposit, expected_paleo_mint) {
        let base_bond_state = {
            validator: this.address,
            microcredits: BigInt('0'),
        };
        let delegator1_bonded = BigInt.asUintN(64, this.credits.bonded.get('pondo_delegator1.aleo')?.microcredits ||
            base_bond_state?.microcredits);
        let delegator2_bonded = BigInt.asUintN(64, this.credits.bonded.get('pondo_delegator2.aleo')?.microcredits ||
            base_bond_state?.microcredits);
        let delegator3_bonded = BigInt.asUintN(64, this.credits.bonded.get('pondo_delegator3.aleo')?.microcredits ||
            base_bond_state?.microcredits);
        let delegator4_bonded = BigInt.asUintN(64, this.credits.bonded.get('pondo_delegator4.aleo')?.microcredits ||
            base_bond_state?.microcredits);
        let delegator5_bonded = BigInt.asUintN(64, this.credits.bonded.get('pondo_delegator5.aleo')?.microcredits ||
            base_bond_state?.microcredits);
        let base_unbond_state = {
            microcredits: BigInt('0'),
            height: BigInt('0'),
        };
        let delegator1_unbonding = BigInt.asUintN(64, this.credits.unbonding.get('pondo_delegator1.aleo')?.microcredits ||
            base_unbond_state?.microcredits);
        let delegator2_unbonding = BigInt.asUintN(64, this.credits.unbonding.get('pondo_delegator2.aleo')?.microcredits ||
            base_unbond_state?.microcredits);
        let delegator3_unbonding = BigInt.asUintN(64, this.credits.unbonding.get('pondo_delegator3.aleo')?.microcredits ||
            base_unbond_state?.microcredits);
        let delegator4_unbonding = BigInt.asUintN(64, this.credits.unbonding.get('pondo_delegator4.aleo')?.microcredits ||
            base_unbond_state?.microcredits);
        let delegator5_unbonding = BigInt.asUintN(64, this.credits.unbonding.get('pondo_delegator5.aleo')?.microcredits ||
            base_unbond_state?.microcredits);
        let delegator1_account = BigInt.asUintN(64, this.credits.account.get('pondo_delegator1.aleo') || BigInt('0'));
        let delegator2_account = BigInt.asUintN(64, this.credits.account.get('pondo_delegator2.aleo') || BigInt('0'));
        let delegator3_account = BigInt.asUintN(64, this.credits.account.get('pondo_delegator3.aleo') || BigInt('0'));
        let delegator4_account = BigInt.asUintN(64, this.credits.account.get('pondo_delegator4.aleo') || BigInt('0'));
        let delegator5_account = BigInt.asUintN(64, this.credits.account.get('pondo_delegator5.aleo') || BigInt('0'));
        let total_bonded = BigInt.asUintN(64, delegator1_bonded +
            delegator2_bonded +
            delegator3_bonded +
            delegator4_bonded +
            delegator5_bonded);
        let total_account = BigInt.asUintN(64, delegator1_account +
            delegator2_account +
            delegator3_account +
            delegator4_account +
            delegator5_account);
        let total_unbonding = BigInt.asUintN(64, delegator1_unbonding +
            delegator2_unbonding +
            delegator3_unbonding +
            delegator4_unbonding +
            delegator5_unbonding);
        let bonded_withdrawals = BigInt.asUintN(64, this.balances.get(this.BONDED_WITHDRAWALS));
        assert(bonded_withdrawals !== undefined);
        let total_delegated = BigInt.asIntN(64, total_bonded + total_account + total_unbonding - bonded_withdrawals);
        let currently_delegated = BigInt.asUintN(64, this.balances.get(this.DELEGATED_BALANCE));
        assert(currently_delegated !== undefined);
        let current_owed_commission = BigInt.asUintN(64, this.owed_commission.get(BigInt('0')));
        assert(current_owed_commission !== undefined);
        let total_paleo_pool = BigInt.asUintN(128, this.multi_token_support_program.registered_tokens.get(this.PALEO_TOKEN_ID).supply +
            current_owed_commission -
            expected_paleo_mint);
        assert(total_paleo_pool !== undefined);
        let rewards = BigInt.asIntN(64, total_delegated > currently_delegated
            ? total_delegated - currently_delegated
            : BigInt('0'));
        let new_commission = BigInt.asUintN(64, this.inline_get_commission(rewards, this.PROTOCOL_FEE));
        currently_delegated += rewards - new_commission;
        let core_protocol_account = BigInt.asUintN(64, this.credits.account.get(this.address) || BigInt('0'));
        let reserved_for_withdrawal = BigInt.asUintN(64, this.balances.get(this.CLAIMABLE_WITHDRAWALS));
        assert(reserved_for_withdrawal !== undefined);
        let current_state = BigInt.asUintN(8, this.protocol_state.get(this.PROTOCOL_STATE_KEY));
        assert(current_state !== undefined);
        let deposit_pool = BigInt.asUintN(64, current_state == this.NORMAL_STATE
        ...
        let new_commission_paleo = BigInt.asUintN(64, this.inline_calculate_new_paleo(currently_delegated, deposit_pool, new_commission, total_paleo_pool));
        ...
        let paleo_for_deposit = BigInt.asUintN(64, this.inline_calculate_new_paleo(currently_delegated, deposit_pool, credits_deposit, total_paleo_pool));
        this.computed_paleo_for_deposit = paleo_for_deposit;
        assert(paleo_for_deposit >= expected_paleo_mint, 'Assert that the transition did not mint too much pALEO');
    }
    inline_get_commission(rewards, commission_rate) {
        let commission = BigInt.asUintN(128, (rewards * commission_rate) / this.PRECISION_UNSIGNED);
        let commission_64 = BigInt.asUintN(64, commission);
        return commission_64;
    }
    inline_calculate_new_paleo(bonded_balance, existing_deposit_pool, deposit, paleo) {
        let full_balance = BigInt.asUintN(128, bonded_balance + existing_deposit_pool);
        let new_total_paleo = BigInt.asUintN(128, (paleo * (full_balance + deposit)) / full_balance);
        let diff = BigInt.asUintN(128, new_total_paleo - paleo);
        let paleo_to_mint = BigInt.asUintN(64, diff);
        return paleo_to_mint;
    }
    // Note: requires the caller to create an allowance for the contract first
    deposit_public(credits_deposit, expected_paleo_mint, referrer) {
        assert(expected_paleo_mint >= BigInt('1'), 'Assert that the expected pALEO mint is at least 1 microcredit');
        // Transfer ALEO to pool
        this.multi_token_support_program.signer = this.signer;
        this.multi_token_support_program.caller = 'pondo_core_protocol.aleo';
        this.multi_token_support_program.transfer_from_public(this.CREDITS_TOKEN_ID, this.caller, this.address, credits_deposit);
        this.multi_token_support_program.signer = this.signer;
        this.multi_token_support_program.caller = 'pondo_core_protocol.aleo';
    ...
    finalize_deposit_public(credits_deposit, expected_paleo_mint) {
        let base_bond_state = {
            validator: this.address,
            microcredits: BigInt('0'),
        };
    ...
    distribute_deposits(transfer_amounts) {
    ...
    finalize_distribute_deposits() {
        ...
        let currently_delegated = BigInt.asUintN(64, this.balances.get(this.DELEGATED_BALANCE));
        assert(currently_delegated !== undefined);
        let account_balance = BigInt.asUintN(64, this.credits.account.get(this.address) || BigInt('0'));
        let reserved_for_withdrawal = BigInt.asUintN(64, this.balances.get(this.CLAIMABLE_WITHDRAWALS));
        assert(reserved_for_withdrawal !== undefined);
        let liquidity_pool = BigInt.asUintN(64, account_balance - reserved_for_withdrawal);
        let optimal_liquidity = BigInt.asUintN(64, this.inline_calculate_optimal_liquidity(currently_delegated));
        assert(liquidity_pool >= optimal_liquidity);
        // Confirm each delegator is already bonded and in the correct state
        let delegator1_state = BigInt.asUintN(8, this.pondo_delegator1.state_mapping.get(BigInt('0')));
        assert(delegator1_state !== undefined);
        let delegator2_state = BigInt.asUintN(8, this.pondo_delegator2.state_mapping.get(BigInt('0')));
        assert(delegator2_state !== undefined);
        let delegator3_state = BigInt.asUintN(8, this.pondo_delegator3.state_mapping.get(BigInt('0')));
        assert(delegator3_state !== undefined);
        let delegator4_state = BigInt.asUintN(8, this.pondo_delegator4.state_mapping.get(BigInt('0')));
        assert(delegator4_state !== undefined);
        let delegator5_state = BigInt.asUintN(8, this.pondo_delegator5.state_mapping.get(BigInt('0')));
        assert(delegator5_state !== undefined);
        assert(delegator1_state == this.BOND_ALLOWED ||
            delegator1_state == this.UNBOND_NOT_ALLOWED, 'Assert that delegator1 is in the correct state');
        assert(delegator2_state == this.BOND_ALLOWED ||
            delegator2_state == this.UNBOND_NOT_ALLOWED, 'Assert that delegator2 is in the correct state');
        assert(delegator3_state == this.BOND_ALLOWED ||
            delegator3_state == this.UNBOND_NOT_ALLOWED, 'Assert that delegator3 is in the correct state');
        assert(delegator4_state == this.BOND_ALLOWED ||
            delegator4_state == this.UNBOND_NOT_ALLOWED, 'Assert that delegator4 is in the correct state');
        assert(delegator5_state == this.BOND_ALLOWED ||
            delegator5_state == this.UNBOND_NOT_ALLOWED, 'Assert that delegator5 is in the correct state');
    }
    inline_calculate_optimal_liquidity(total_balance) {
        let min_liquidity = BigInt.asUintN(128, (total_balance * this.MIN_LIQUIDITY_PERCENT) / this.PRECISION_UNSIGNED);
        let optimal_liquidity = BigInt.asUintN(64, min_liquidity > this.MAX_GUARANTEED_LIQUIDITY
    ...
    // WITHDRAW FUNCTIONS
    // -------------------
    instant_withdraw_public(paleo_burn_amount, withdrawal_credits) {
    ...
    finalize_instant_withdraw_public(paleo_burn_amount, withdrawal_credits, caller) {
        // Block instant withdrawals during a rebalance
        let current_state = BigInt.asUintN(8, this.protocol_state.get(this.PROTOCOL_STATE_KEY));
        assert(current_state !== undefined);
        assert(current_state == this.NORMAL_STATE, 'Assert that the protocol is not in a rebalancing state');
        let has_withdrawal = this.withdrawals.has(caller);
        ...
        let current_owed_commission = BigInt.asUintN(64, this.owed_commission.get(BigInt('0')));
        assert(current_owed_commission !== undefined);
        let paleo_minted_post_burn = BigInt.asUintN(128, this.multi_token_support_program.registered_tokens.get(this.PALEO_TOKEN_ID).supply + current_owed_commission);
        assert(paleo_minted_post_burn !== undefined);
        let total_paleo_minted = BigInt.asUintN(128, paleo_minted_post_burn + paleo_burn_amount);
        let rewards = BigInt.asIntN(64, total_delegated > currently_delegated
            ? total_delegated - currently_delegated
            : BigInt('0'));
        let new_commission = BigInt.asUintN(64, this.inline_get_commission(rewards, this.PROTOCOL_FEE));
        ...
        let reserved_for_withdrawal = BigInt.asUintN(64, this.balances.get(this.CLAIMABLE_WITHDRAWALS));
        assert(reserved_for_withdrawal !== undefined);
        let deposit_pool = BigInt.asUintN(64, core_protocol_account - reserved_for_withdrawal + withdrawal_credits);
        // Update owed commission balance
        let new_commission_paleo = BigInt.asUintN(64, this.inline_calculate_new_paleo(currently_delegated, deposit_pool, new_commission, total_paleo_minted));
        current_owed_commission += new_commission_paleo;
        total_paleo_minted += new_commission_paleo;
        currently_delegated += new_commission;
        // Calculate full pool size
        let full_pool = BigInt.asUintN(128, currently_delegated + deposit_pool);
        // Calculate credits value of burned pALEO
        let withdrawal_fee = BigInt.asUintN(64, this.inline_calculate_withdraw_fee(paleo_burn_amount));
        let net_burn_amount = BigInt.asUintN(64, paleo_burn_amount - withdrawal_fee);
        let withdrawal_calculation = BigInt.asUintN(128, (net_burn_amount * full_pool) / total_paleo_minted);
        assert(withdrawal_credits <= withdrawal_calculation, 'Assert that the withdrawal amount was at most the calculated amount');
    ...
    inline_calculate_withdraw_fee(paleo_burn_amount) {
        let fee_calc = BigInt.asUintN(128, (paleo_burn_amount * this.INSTANT_WITHDRAW_FEE) / this.PRECISION_UNSIGNED);
        let fee = BigInt.asUintN(64, fee_calc);
        return fee;
    }
    withdraw_public(paleo_burn_amount) {
        // Burn pALEO for withdrawal
        this.pondo_staked_aleo_token.signer = this.signer;
        this.pondo_staked_aleo_token.caller = 'pondo_core_protocol.aleo';
        this.pondo_staked_aleo_token.burn_public(paleo_burn_amount, this.caller);
    ...
    finalize_withdraw_public(paleo_burn_amount, caller) {
        ...
        let batch_height = this.inline_get_withdrawal_batch(this.block.height);
        let withdrawal_state_value = {
    ...
    inline_get_withdrawal_batch(height) {
        ...
        let withdrawal_batch = BigInt.asUintN(32, min_block_height / this.BLOCKS_PER_EPOCH);
        // Withdrawals are processed at the start of the next epoch
        let claim_block = BigInt.asUintN(32, (withdrawal_batch + BigInt('1')) * this.BLOCKS_PER_EPOCH +
            this.REBALANCE_PERIOD +
            BigInt('1'));
        return [withdrawal_batch, claim_block];
    }
    claim_withdrawal_public(owner, amount) {
    ...
    finalize_claim_withdrawal_public(owner, amount) {
        // Update withdrawal state
        let withdrawal = this.withdrawals.get(owner);
        assert(withdrawal !== undefined);
        assert(withdrawal.claim_block < this.block.height, 'make sure the withdrawal is claimable');
    ...
    finalize_prep_rebalance() {
        ...
        let current_epoch = BigInt.asUintN(32, this.block.height / this.BLOCKS_PER_EPOCH);
        let last_rebalance = BigInt.asUintN(32, this.last_rebalance_epoch.get(BigInt('0')) || BigInt('4294967295'));
        ...
        let blocks_into_epoch = BigInt.asUintN(32, this.block.height % this.BLOCKS_PER_EPOCH);
        assert(blocks_into_epoch < this.REBALANCE_PERIOD ||
            last_rebalance == BigInt('4294967295'), 'rebalance is allowed during the first day of a new epoch');
        let top_validators = this.pondo_oracle.top_validators.get(BigInt('0')) || [
            'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc',
            'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc',
            'aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc',
    ...
    rebalance_retrieve_credits(transfer_amounts, commission_mint) {
        this.pondo_delegator1.signer = this.signer;
        this.pondo_delegator1.caller = 'pondo_core_protocol.aleo';
    ...
    finalize_rebalance_retrieve_credits(transfer_amounts, commission_mint) {
        let full_balance = BigInt.asUintN(64, transfer_amounts[0] +
            transfer_amounts[1] +
            transfer_amounts[2] +
            transfer_amounts[3] +
            transfer_amounts[4]);
        let current_balance = BigInt.asUintN(64, this.balances.get(this.DELEGATED_BALANCE));
        ...
        let total_paleo_minted = BigInt.asUintN(128, this.multi_token_support_program.registered_tokens.get(this.PALEO_TOKEN_ID).supply +
            current_owed_commission -
            commission_mint);
        assert(total_paleo_minted !== undefined);
        let rewards = BigInt.asIntN(64, full_balance > current_balance
        ...
        let reserved_for_withdrawal = BigInt.asUintN(64, this.balances.get(this.CLAIMABLE_WITHDRAWALS));
        assert(reserved_for_withdrawal !== undefined);
        let deposit_pool = BigInt.asUintN(64, core_protocol_account - full_balance - reserved_for_withdrawal);
        let new_commission_paleo = BigInt.asUintN(64, this.inline_calculate_new_paleo(current_balance, deposit_pool, new_commission, total_paleo_minted));
        ...
        let current_withdrawal_batch = BigInt.asUintN(64, this.withdrawal_batches.get(current_epoch - BigInt('1')) || BigInt('0'));
    ...
    rebalance_redistribute(validators, transfer_amounts) {
    ...
    finalize_rebalance_redistribute(validators, transfer_amounts) {
        // Check that the new validator set is correct
        let next_validator_set = this.validator_set.get(this.NEXT_VALIDATOR_SET);
        this.validator_set.set(this.CURRENT_VALIDATOR_SET, next_validator_set);
        this.validator_set.delete(this.NEXT_VALIDATOR_SET);
        for (let i = 0; i < 5; i++) {
            assert(validators[i].validator == next_validator_set[i].validator &&
                validators[i].commission == next_validator_set[i].commission, 'ensure that the new validator set is correct');
        }
        // Check that each validator has the correct portion of credits
        let delegator_allocation = this.pondo_oracle.delegator_allocation.get(BigInt('0')) || [
            this.PORTION_1,
            this.PORTION_2,
            this.PORTION_3,
            this.PORTION_4,
        ...
        let total_credits = BigInt.asUintN(64, transfer_amounts[0] +
            transfer_amounts[1] +
            transfer_amounts[2] +
            transfer_amounts[3] +
            transfer_amounts[4]);
        let total_credits_128 = BigInt.asUintN(128, total_credits);
        let validator1_portion = BigInt.asUintN(128, (transfer_amounts[0] * this.PRECISION_UNSIGNED) / total_credits_128);
        let validator2_portion = BigInt.asUintN(128, (transfer_amounts[1] * this.PRECISION_UNSIGNED) / total_credits_128);
        let validator3_portion = BigInt.asUintN(128, (transfer_amounts[2] * this.PRECISION_UNSIGNED) / total_credits_128);
        let validator4_portion = BigInt.asUintN(128, (transfer_amounts[3] * this.PRECISION_UNSIGNED) / total_credits_128);
        let validator5_portion = BigInt.asUintN(128, (transfer_amounts[4] * this.PRECISION_UNSIGNED) / total_credits_128);
        ...
        assert(reserved_for_withdrawal !== undefined);
        let liquidity_pool = BigInt.asUintN(64, account_balance - reserved_for_withdrawal);
        let optimal_liquidity = BigInt.asUintN(64, this.inline_calculate_optimal_liquidity(total_credits_128));
        assert(liquidity_pool >= optimal_liquidity, 'ensure that liquidity pool is at least optimal liquidity');
        assert(liquidity_pool <= optimal_liquidity + BigInt('250'), 'ensure that liquidity pool is close to optimal liquidity');
    ...
    finalize_set_oracle_tvl(tvl) {
        // Ensure the tvl matches what exists in the core protocol
        // Get all of the delegator balances
        let delegator1_balance = BigInt.asUintN(64, this.credits.account.get('pondo_delegator1.aleo') || BigInt('0'));
        let delegator2_balance = BigInt.asUintN(64, this.credits.account.get('pondo_delegator2.aleo') || BigInt('0'));
        let delegator3_balance = BigInt.asUintN(64, this.credits.account.get('pondo_delegator3.aleo') || BigInt('0'));
        let delegator4_balance = BigInt.asUintN(64, this.credits.account.get('pondo_delegator4.aleo') || BigInt('0'));
        let delegator5_balance = BigInt.asUintN(64, this.credits.account.get('pondo_delegator5.aleo') || BigInt('0'));
        // Get all of the bonded balances
        let default_bond_state = {
            validator: this.address,
            microcredits: BigInt('0'),
        };
        let delegator1_bonded = BigInt.asUintN(64, this.credits.bonded.get('pondo_delegator1.aleo')?.microcredits ||
        ...
        let core_protocol_balance = BigInt.asUintN(64, this.credits.account.get(this.address) || BigInt('0'));
        // Calculate the total tvl
        let total_tvl = BigInt.asUintN(64, delegator1_balance +
            delegator2_balance +
            delegator3_balance +
            delegator4_balance +
            delegator5_balance +
            ...
            delegator4_unbonding +
            delegator5_unbonding);
        assert(total_tvl >= (tvl * BigInt('98')) / BigInt('100') &&
            total_tvl <= (tvl * BigInt('102')) / BigInt('100'), 'Assert that the total tvl matches the tvl provided within a margin of error of 2%');
    }
}
