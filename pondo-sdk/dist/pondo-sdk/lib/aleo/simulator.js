import { credits, coreProtocol, delegator1, delegator2, delegator3, delegator4, delegator5, MTSP, oracle, pALEO, PNDO, } from '../../../simulator/src/contracts/pondoProgramsIndex.js';
...
export async function initializeContracts() {
    ...
    coreProtocolInstance = new coreProtocol(delegator5Instance, delegator4Instance, delegator3Instance, delegator2Instance, delegator1Instance, PNDOInstance, pALEOInstance, oracleInstance, MTSPInstance, creditsInstance);
...
export async function initializeMappings(rpcProvider, presetMappingKeys, postFetchReplace) {
    if (postFetchReplace == null)
        postFetchReplace = {};
    const presetMappingValues = await getPresetMappingValues(rpcProvider, presetMappingKeys, postFetchReplace);
...
function initializeContractMappings(contract, presetMappingValues) {
    for (const [mappingName, mapping] of Object.entries(contract)) {
        if (!(mapping instanceof Map))
            continue;
        mapping.get = initializeMappingGetter(contract, mappingName, mapping, mapping.get.bind(mapping), presetMappingValues);
...
async function getPresetMappingValues(rpcProvider, presetMappingKeys, postFetchReplace) {
    const presetMappingValuesArray = await Promise.all(presetMappingKeys.map(async ([programId, mappingName, key]) => ([
        programId,
        mappingName,
        snarkVMToMapping(key, postFetchReplace),
        snarkVMToMapping(await getMappingValueOrUndefined(rpcProvider, { programId, mappingName, key }), postFetchReplace)
    ])));
    return presetMappingValuesArray.reduce((prev, [programId, mappingName, key, value]) => {
        if (prev[programId] == null)
            prev[programId] = {};
        if (prev[programId][mappingName] == null)
            prev[programId][mappingName] = {};
...
function toMappingRec(jsValue, postFetchReplace) {
    ...
    if (typeof jsValue === 'string') {
        ...
        return Object.fromEntries(Object.entries(jsValue).map(([key, val]) => ([toMappingRec(key), toMappingRec(val)])));
    }
    return jsValue;
}
async function getMappingValueOrUndefined(rpcProvider, { programId, mappingName, key }) {
    try {
