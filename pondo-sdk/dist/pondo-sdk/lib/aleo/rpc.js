import { pondoProgramToCode } from '../../../pondo-bot/src/compiledPrograms.js';
const RPC_HEADERS = {
...
export class LiveRpcProvider {
    ...
    // Endpoints
    async aleoTransactionsForProgram({ programId, functionName, page, maxTransactions }) {
        return await this.callRPC("aleoTransactionsForProgram", { programId, functionName, page, maxTransactions });
    }
...
LiveRpcProvider.from_url = async (url) => {
    ...
    if (!status_res?.online) {
        throw new Error(`RPC unavailable at '${url}'.`, status_res?.statusTitle, status_res?.statusMessage);
    }
    return instance;
};
