...
const extract_value = (segment) => {
    ...
    if (segment.startsWith('{') && segment.endsWith('}')) {
        ...
        return segment.slice(1, -1).split(',').map(item => convert_snarkvm_value(item.trim().replace(/(\.private|\.public)$/, '')));
    }
    return convert_snarkvm_value(segment.replace(/(\.private|\.public)$/, ''));
};
export const snarkVMToJs = (snarkVMValue) => {
    if (snarkVMValue == null) {
        return snarkVMValue;
    }
    return extract_value(snarkVMValue);
};
export function structStringToAttributes(structString) {
    return Object.entries(extractAleoTypes(structString)).map(([name, type]) => ({ name, type }));
}
...
const extractAleoTypes = (segment) => {
    ...
    if (segment.startsWith('{') && segment.endsWith('}')) {
        ...
        return segment.slice(1, -1).split(',').map(item => getAleoType(item.trim().replace(/(\.private|\.public)$/, '')));
    }
...
export async function hashNonNestedStruct(structString) {
    ...
    const structAttributeInstructions = structAttributes
  ...
  `;
    ...
    const res = await executeProgramOffchain(programSource, functionName, inputs);
    return res[0];
}
